...
def make_parser():
    ...
    parser.add_argument(
        "-l", "--log_name", default="exp", type=str, help="log_name"
    )
    parser.add_argument(
...
def main():
    ...
    log_dir = "logs/{}".format(os.path.basename(args.file).split(".")[0] + '_' + args.log_name)
...
def worker(args):
    ...
    if args.weight_file.endswith('.pth'):
        ...
        for k, v in weights['state_dict'].items():
            if 'tracked' in k or 'fc_cls' in k or 'fc_reg' in k:
                continue
            elif 'backbone' in k:
                weight_new[k[:9] + 'bottom_up.' + k[9:]] = v.numpy()
            elif 'neck.lateral_convs.0.conv' in k:  # param `backbone.fpn_lateral2.bias` shape mismatch, should be (1, 256, 1, 1), get (256,)
                if 'bias' in k:
                    weight_new['backbone.fpn_lateral2.' + k.split('.')[-1]] = v.numpy().reshape(1, -1, 1, 1)
                else:
                    weight_new['backbone.fpn_lateral2.' + k.split('.')[-1]] = v.numpy()
            elif 'neck.lateral_convs.1.conv' in k:
                if 'bias' in k:
                    weight_new['backbone.fpn_lateral3.' + k.split('.')[-1]] = v.numpy().reshape(1, -1, 1, 1)
                else:
                    weight_new['backbone.fpn_lateral3.' + k.split('.')[-1]] = v.numpy()
            elif 'neck.lateral_convs.2.conv' in k:
                if 'bias' in k:
                    weight_new['backbone.fpn_lateral4.' + k.split('.')[-1]] = v.numpy().reshape(1, -1, 1, 1)
                else:
                    weight_new['backbone.fpn_lateral4.' + k.split('.')[-1]] = v.numpy()
            elif 'neck.lateral_convs.3.conv' in k:
                if 'bias' in k:
                    weight_new['backbone.fpn_lateral5.' + k.split('.')[-1]] = v.numpy().reshape(1, -1, 1, 1)
                else:
                    weight_new['backbone.fpn_lateral5.' + k.split('.')[-1]] = v.numpy()

            elif 'neck.fpn_convs.0.conv' in k:
                if 'bias' in k:
                    weight_new['backbone.fpn_output2.' + k.split('.')[-1]] = v.numpy().reshape(1, -1, 1, 1)
                else:
                    weight_new['backbone.fpn_output2.' + k.split('.')[-1]] = v.numpy()
            elif 'neck.fpn_convs.1.conv' in k:
                if 'bias' in k:
                    weight_new['backbone.fpn_output3.' + k.split('.')[-1]] = v.numpy().reshape(1, -1, 1, 1)
                else:
                    weight_new['backbone.fpn_output3.' + k.split('.')[-1]] = v.numpy()
            elif 'neck.fpn_convs.2.conv' in k:
                if 'bias' in k:
                    weight_new['backbone.fpn_output4.' + k.split('.')[-1]] = v.numpy().reshape(1, -1, 1, 1)
                else:
                    weight_new['backbone.fpn_output4.' + k.split('.')[-1]] = v.numpy()
            elif 'neck.fpn_convs.3.conv' in k:
                if 'bias' in k:
                    weight_new['backbone.fpn_output5.' + k.split('.')[-1]] = v.numpy().reshape(1, -1, 1, 1)
                else:
                    weight_new['backbone.fpn_output5.' + k.split('.')[-1]] = v.numpy()

            elif 'rpn_conv' in k:
                if 'bias' in k:
                    weight_new['rpn.rpn_conv.' + k.split('.')[-1]] = v.numpy().reshape(1, -1, 1, 1)
                else:
                    weight_new['rpn.rpn_conv.' + k.split('.')[-1]] = v.numpy()
            elif 'rpn_cls' in k:
                if 'bias' in k:
                    weight_new['rpn.rpn_cls_score.' + k.split('.')[-1]] = v.numpy().reshape(1, -1, 1, 1)
                else:
                    weight_new['rpn.rpn_cls_score.' + k.split('.')[-1]] = v.numpy()
            elif 'rpn_reg' in k:
                if 'bias' in k:
                    weight_new['rpn.rpn_bbox_offsets.' + k.split('.')[-1]] = v.numpy().reshape(1, -1, 1, 1)
                else:
                    weight_new['rpn.rpn_bbox_offsets.' + k.split('.')[-1]] = v.numpy()

            elif 'shared_fcs.0' in k:
                weight_new['rcnn.fc1.' + k.split('.')[-1]] = v.numpy()
            elif 'shared_fcs.1' in k:
                weight_new['rcnn.fc2.' + k.split('.')[-1]] = v.numpy()
            # else:
            #     assert False, print(k)
    ...
    for epoch in range(model.cfg.max_epoch):
        ...
        if dist.get_rank() == 0:
            save_path = "logs/{}/epoch_{}.pkl".format(
                ...
                os.path.basename(args.file).split(".")[0] + '_' + args.log_name, epoch
            )
            mge.save(
...
def adjust_learning_rate(optimizer, epoch, step, cfg, args):
    base_lr = (
        ...
        cfg.basic_lr * args.batch_size * args.devices * (  # clw modify
            cfg.lr_decay_rate
    ...
    train_dataloader = DataLoader(
        ...
        transform=T.Compose(
            transforms=[
                ...
                # T.RandomResizedCrop(output_size=(2048, 2048), scale_range=(0.6, 1.0)),
