...
from tools.utils import DetEvaluator, InferenceSampler, import_from_file, py_cpu_nms
...
def make_parser():
    ...
    parser.add_argument(
        "-l", "--log_name", default="exp", type=str, help="net description file"
    )
    parser.add_argument(
    ...
    for epoch_num in range(args.start_epoch, args.end_epoch + 1):
        if args.weight_file:
            ...
            weight_file = "logs/{}/epoch_{}.pkl".format(
                ...
                os.path.basename(args.file).split(".")[0] + '_' + args.log_name, epoch_num
            )
        ...
        if args.devices > 1:
            ...
            num_imgs = dict(coco=5000, objects365=30000, traffic511=584)  # test set
...
def worker(
    ...
    for data in test_loader:
        image, im_info = DetEvaluator.process_inputs(  # note: resize img to size in cfg
            data[0][0],
        ...
        if FLIP_TEST:
            ...
            if tmp_res.size > 0:
                if pred_res.size == 0:
                    ...
                    ori_h, ori_w = im_info[0][2], im_info[0][3]
                    tmp_res[:, 0], tmp_res[:, 2] = ori_w - tmp_res[:, 2], ori_w - tmp_res[:, 0]
                    pred_res = np.concatenate((pred_res, tmp_res), axis=0)

        ### scale 2
        if MULTI_SCALE_TEST:
            image, im_info = DetEvaluator.process_inputs(  # note: resize img to size in cfg
                data[0][0],
                model.cfg.train_image_short_size[len(model.cfg.train_image_short_size)-1] * 1.1,
                #2000,
                model.cfg.test_image_max_size,
            )
            tmp_res = evaluator.predict(
            ...
            image, im_info = DetEvaluator.process_inputs(  # note: resize img to size in cfg
                data[0][0],
                (model.cfg.train_image_short_size[0] + model.cfg.train_image_short_size[len(model.cfg.train_image_short_size)-1])/2,
                model.cfg.test_image_max_size,
            )
            tmp_res = evaluator.predict(
                image=mge.tensor(image),
        ...
        # nms
        if pred_res.size > 0:
            if model.cfg.test_nms == -1:
                keep = pred_res[:, 4].argsort()[::-1]
            else:
                assert 0 < model.cfg.test_nms <= 1.0
                keep = py_cpu_nms(pred_res, model.cfg.test_nms)
            pred_res = pred_res[keep]
            # dtboxes_all.extend(results)
